...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        ...
        if entry['pre_processed_frames_rpn_ret'] is not None:
            ...
            assert (pre_processed_frames_rpn_ret['rois'][:, 3] >= pre_processed_frames_rpn_ret['rois'][:, 1]).all(), \
                    '{}'.format(pre_processed_frames_rpn_ret['rois'])
        else:
            pre_processed_frames_rpn_ret = None
        # now flip
        flipped_entry = {}
        dont_copy = ('boxes', 'sbj_gt_boxes', 'obj_gt_boxes', \
                        'segms', 'gt_keypoints', 'flipped', 'pre_processed_frames_rpn_ret')

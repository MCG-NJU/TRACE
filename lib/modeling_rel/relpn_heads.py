...
class single_scale_relpn_outputs(nn.Module):
    ...
    def forward(self, det_rois, det_labels, det_scores, im_info, dataset_name, roidb=None, lim_score=1.0):
        """
        ...
        if ((not self.training) or cfg.USE_BASELINE.FULL_GRAPH_MSP) and (dataset_name.find('ag') >= 0 and len(person_inds) > 0): ###!!!
        ...
        if self.training:
            ...
            blobs_out = self.RelPN_GenerateProposalLabels(sbj_rois, obj_rois, det_rois, roidb, im_info, sbj_inds, obj_inds, det_labels)

...
def parse_args():
    ...
    parser.add_argument(
        '--clip_gradient', help='Clip gradient',
        action='store_true')
    parser.add_argument(
        '--eval_loss', help='Used to check loss on test set',
        action='store_true')
    parser.add_argument(
        '--get_frame_rois', dest='get_frame_rois', help='get frame rois as pre-processing', action='store_true')
    parser.add_argument(
        '--seed',
        help='random seed',
        default=1, type=int)
...
def main(args):
    ...
    if args.dataset == "vrd":
        ...
        cfg.MODEL.OBJ_CLASS_FILE_NAME = os.path.join(cfg.ROOT_DIR, 'data/vrd/objects.json')
    ...
    if args.fp16:
        maskRCNN, optimizer = amp.initialize(maskRCNN, optimizer, opt_level="O1")    ###!!!apex
                ...
                for inner_iter in range(args.iter_size):
                    ...
                    if cfg.TEST.GET_FRAME_ROIS:
                        net_outputs['pre_processed_frames_rpn_ret'][0]['rois'] = \
                                net_outputs['pre_processed_frames_rpn_ret'][0]['rois'].data.cpu().numpy()
                        net_outputs['pre_processed_frames_rpn_ret'][0]['labels_int32'] = \
                                net_outputs['pre_processed_frames_rpn_ret'][0]['labels_int32'].data.cpu().numpy()
                        net_outputs['pre_processed_frames_rpn_ret'][1] = \
                                net_outputs['pre_processed_frames_rpn_ret'][1].data.cpu().numpy()
                        net_outputs['pre_processed_temporal_roi'] = \
                                net_outputs['pre_processed_temporal_roi'].data.cpu().numpy()
                        save_f_file = {'pre_processed_frames_rpn_ret': net_outputs['pre_processed_frames_rpn_ret'],
                                        'pre_processed_temporal_roi': net_outputs['pre_processed_temporal_roi']}
                        
                        f_file_path = os.path.abspath(cfg.FRAME_PRE_PROCESSING_SAVED_PATH)
                        #print(f_file_path)
                        ...
                        if (step + 1) % CHECKPOINT_PERIOD == 0:
                            ...
                            while not res_q.empty():
                                ...
                                #print(save_f_file)
                                if not os.path.exists(os.path.join(f_file_path, roi_file_name + '.pkl')):
                                    with open(os.path.join(f_file_path, roi_file_name + '.pkl'), 'wb') as f:
                    ...
                    if args.clip_gradient:
                        net_utils.clip_gradient(maskRCNN, 1.)
                optimizer.step()
